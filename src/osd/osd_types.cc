...
void pg_pool_t::dump(Formatter *f) const
{
  ...
  for (set<string>::const_iterator p = tiers.begin(); p != tiers.end(); ++p)
    f->dump_string("pool", *p);
  f->close_section();
  f->dump_string("tier_of", tier_of);
  f->dump_string("overlay", overlay);
...
void pg_pool_t::generate_test_instances(list<pg_pool_t*>& o)
{
  ...
  a.tiers.insert("cache_pool");
  a.tiers.insert("other_cache_pool");
  a.tier_of = "data pool";
  a.cache_mode = CACHEMODE_WRITEBACK;
  a.overlay = "cache_pool_overlay";
  o.push_back(new pg_pool_t(a));
}
